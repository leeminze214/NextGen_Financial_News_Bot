...
async function articles(msg,stock){
  ...
  var req = 'https://cloud.iexapis.com/stable/stock/'+stock+'/news/last/5?token='+iextoken
  ...
  filter(embed,real_links,priorities);
msg.reply(embed)
};

function filter(embed,real_links,priorities){
  var count = 0;
  for (i = real_links.length-1; i > 0; i-= 1) {
    ...
    console.log(real_links[i].headline);
    count++;
    }
  }
};
